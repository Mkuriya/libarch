...
<section class="max-w-screen-xl p-6 mx-auto bg-indigo-600 rounded-md shadow-md dark:bg-gray-800 ">
    ...
    <form action="/admin/dashboard/admin/edit/{{$admin->id}}" method="POST" enctype="multipart/form-data">
                ...
                <div>
                    ...
                    <label for="file" class="relative flex min-h-[150px] items-center justify-center rounded-md border border-dashed border-[#e0e0e0] text-center">
                        <div id="imagePreviewContainer">
                            <img id="imagePreview" src="{{asset('storage/' . $admin->photo) }}" class="rounded-md object-cover h-48 w-96" />
                        </div>
                    </label>
                </div>
            </div>
        ...
        <div class="flex justify-end mt-6">
            <button type="submit" class="w-20 py-2 leading-5 text-white transition-colors duration-200 transform bg-whitebg rounded-md hover:bg-gray-700 focus:outline-none focus:bg-gray-600">Update</button>
            <a href="/admin/dashboard/admin"><button type="button" class="ml-4 w-20 px-6 py-2 leading-5 text-white transition-colors duration-200 transform bg-whitebg rounded-md hover:bg-gray-700 focus:outline-none focus:bg-gray-600">Back</button></a>
        </div>
    </form>
